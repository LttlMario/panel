...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === 'discover_discord_roles') {
      ...
      if (!/^\d{15,22}$/.test(guildId)) return reply({ error: 'Guild ID invalid.' }, 400);

      const botToken = kind === 'secondary'
        ? Deno.env.get('DISCORD_SECONDARY_BOT_TOKEN') || Deno.env.get('DISCORD_BOT_TOKEN')
        : Deno.env.get('DISCORD_BOT_TOKEN');
      if (!botToken) return reply({ error: 'Lipseste secretul botului pentru acest server Discord.' }, 400);

      const botHeaders = { Authorization: `Bot ${botToken.trim()}` };
      ...
      if (!guildResponse.ok || !rolesResponse.ok) {
        const status = !guildResponse.ok ? guildResponse.status : rolesResponse.status;
        return reply({ error: `Botul nu poate accesa serverul Discord (HTTP ${status}).` }, 400);
      }

      const guild = await guildResponse.json();
      const discordRoles = await rolesResponse.json();
      const roles = (Array.isArray(discordRoles) ? discordRoles : [])
        .filter((role: any) => String(role.id) !== guildId && !role.managed)
        .map((role: any) => ({ id: String(role.id), name: String(role.name), position: Number(role.position || 0) }))
        .sort((a: any, b: any) => b.position - a.position);

      return reply({ guild: { id: String(guild.id), name: String(guild.name) }, roles });
    }
    ...
    if (body.action === 'diagnose') {
      ...
      const secondaryDiscordConfigured =
        Boolean(config?.discord_client_id_secondary) &&
        Boolean(config?.guild_id_secondary);

      add(
        'discord-config',
        'Discord',
        'Organizație și servere Discord',
        missingConfig.length ? 'error' : 'ok',
        missingConfig.length
          ? `Câmpuri necompletate: ${missingConfig.join(', ')}.`
          : secondaryDiscordConfigured
            ? 'Organizația, Discord Principal și Discord Secundar sunt configurate.'
            : 'Organizația și Discord Principal sunt configurate. Discord Secundar nu este activ.'
      );
    ...
    if (body.action === 'save') {
      ...
      const secondaryClientId = String(config.discord_client_id_secondary || '').trim();
      const secondaryGuildId = String(config.guild_id_secondary || '').trim();
      if (Boolean(secondaryClientId) !== Boolean(secondaryGuildId)) {
        throw new Error('Pentru Discord Secundar completeaza impreuna Client ID si Guild ID.');
      }
      if (secondaryClientId && !/^\d{15,22}$/.test(secondaryClientId)) throw new Error('Client ID Discord Secundar invalid.');
      if (secondaryGuildId && !/^\d{15,22}$/.test(secondaryGuildId)) throw new Error('Guild ID Discord Secundar invalid.');
      ...
      const configRow = {
          ...
          organization_code: config.organization_code || null,
          organization_description: config.organization_description || null,
          organization_logo: config.organization_logo || null,
          organization_banner: config.organization_banner || null,
          ...
          // Discord Secundar
          discord_client_id_secondary: secondaryClientId || null,

          guild_id_secondary: secondaryGuildId || null,
      ...
      const mappingRows = mappings.map((item: any) => ({
              ...
              discord_role_name:
                  String(item.discord_role_name || '').trim(),

              discord_role_id_secondary:
                  item.discord_role_id_secondary
                      ? String(item.discord_role_id_secondary).trim()
                      : null,

              discord_role_name_secondary:
                  item.discord_role_name_secondary
                      ? String(item.discord_role_name_secondary).trim()
                      : null,

              panel_role:
                  String(item.panel_role || '').trim(),

              permission_level:
                  Number(item.permission_level),

              priority:
                  Number(item.permission_level) * 10,
